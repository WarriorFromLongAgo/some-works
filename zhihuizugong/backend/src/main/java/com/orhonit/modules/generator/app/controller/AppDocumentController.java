...
public class AppDocumentController extends AbstractController{
    @Autowired
...
//    @RequiresPermissions("generator:document:delete")
    ...
    /**
     * TODO   APP端 【我的】模块: 公文通知
         * 未读条数和未读列表
     * @param userId
     * @return
     */
    @GetMapping("/UnreadTotalAndList")
    public R UnreadTotalAndList() {
    	Integer userId = getUserId().intValue();
		Map t=documentService.UnreadTotalAndList(userId);
    	return R.ok().put("DocumentEntity", t);
    }
     ...
     */
    @PutMapping("/upread")
    public R updateReamarks(String documentId) {
    	documentService.updateRemarks(documentId);
    	return R.ok();
    }
    ...
    public Map<String,Object> findData(@RequestParam Map<String,Object> params){
        return documentService.findData(params);
    }
