 ...
 */
@RestController
@RequestMapping("generator/documentfile")
...
//    @RequiresPermissions("generator:documentfile:delete")
    ...
    public R uploadFile(@RequestParam("file") MultipartFile file, HttpServletRequest request) {
        ...
        // 文件保存路径
        String Path = uploadConfig.getPath() + "document/";
        String filePath = "/api/image/document/" ;//uploadConfig.getPath() + "ipb-meeting/" + dateStr + "/"
        ...
        try {
            ...
            documentFileService.insert(documentFile);
            return R.ok().put("id", documentFile.getId());
        } catch (IOException e) {
            e.printStackTrace();
            return R.error();
        }
