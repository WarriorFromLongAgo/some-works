...
import com.orhonit.modules.sys.dao.SysRoleDao;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleDao, SysRoleEntity> implements SysRoleService {
	...
	public PageUtils queryPage(Map<String, Object> params) {
		...
		int isapp = 0;
		if(params.get("isapp")!=null) {
			 isapp = Integer.parseInt(params.get("isapp").toString());
		}
		...
		page.setTotal(this.selectCount(
				new EntityWrapper<SysRoleEntity>()
				.like(StringUtils.isNotBlank(roleName),"role_name", roleName)
				.eq(createUserId != null,"create_user_id", createUserId)
				.eq("isapp", isapp)
	...
	private void checkPrems(SysRoleEntity role){
		...
		//查询用户所拥有的菜单列表
		List<Long> menuIdList = sysUserService.queryAllMenuId(role.getCreateUserId(), 1);
