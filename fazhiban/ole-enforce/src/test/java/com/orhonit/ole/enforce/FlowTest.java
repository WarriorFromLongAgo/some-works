...
public class FlowTest {
	...
	public void testFlow() {
		...
		/**
		 * 功能点测试 1. 部署 #完成 createDeployment(); 2. 根据部署名称获取部署Key #完成
		 * getDeploymentList(); 3. 删除一个部署流程 #完成
		 * deleteProcessDefinitionByDeploymentId(); 4. 发起一个流程 #完成 startFlow();
		 * 为了测试起来方便,需要回滚数据 5. 流程与业务数据绑定 #完成 .. 6. 代办事宜 #完成 .. 7. 从案件受理步骤到初步审核
		 * #完成 8. 详细审批记录以及批注信息
		 */

		/**
		 * 代码 this.createDeployment(); String deployName = "案件受理"; String key =
		 * getDeploymentList(deployName); //查看已部署的流程 System.out.println(
		 * "the key is : " + key);
	...
	public void createDeployment() {
		...
		try {
			...
			// File file = new File("classpath:processes/bpmn1tuihui.zip");
			File file = ResourceUtils.getFile("E:\\workflow\\itcast0820Project\\WebRoot\\bpmn\\case.zip");
			/// processes/bpmn1tuihui.zip
			ZipInputStream zipInputStream = new ZipInputStream(new FileInputStream(file));
			processEngine.getRepositoryService().createDeployment()// 创建部署对象
					.name(fileName)// 添加部署名称
...
/*
	...
	public void start() {
		...
		zfryUser.setPersonId("3ff9477907f84b79b7d98f7d12ee1fa5");
		zfryUser.setPersonName("郭子瑛");
		zfryUser.setUserId("25047");
		
		User fzyryUser = new User();
		fzyryUser.setPersonId("f22ee28cc5aa44e18a124739ec34c1f5");
		fzyryUser.setPersonName("潘孝");
		fzyryUser.setUserId("25046");
		
		User shr = new User();
		shr.setPersonName("王文清");
		shr.setUserId("25045");
		
		User spr = new User();
		spr.setPersonName("王贵苍");
		spr.setUserId("25044");
		
		String deptId = "169b3b34bf1c44deb5dbd65491c8b71e";
	...
	public void randomCreateCase(User zf, User fz, User shr, User spr, String deptId) {
		log.info("random create case test is start.");
	...
	public void toJyja(CaseEntity caseInfoDTO, User zf) {
	...
	//随机跑数据，每条路径都能结束
	public void caseRandomFlow(CaseEntity caseInfoDTO, User zf, User shr, User spr,User fzsl,User fzsh,User fzsp,User fycb,User fysh,User fysp,User tzsl,String mode){
		User assignee = null;
		User nextAssignee = null;
		String nextMode = new String();
		CommonParameters.SimpleFlow.AUDIT_OPINION.toString();
		Random rand = new Random();
		String[] nextModeList=null;
		switch (mode) {
		case "1":
			//不予处理
			assignee = zf;
		...
		case "3":
			...
			nextAssignee = zf;
			//下一步可走的流程列表
			nextModeList = new String[]{"4","5"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			break;
		case "4":
			//给与警告
			assignee = zf;
		...
		case "19":
			//法制审核意见
			assignee = fzsh;
			nextModeList = new String[]{"20","21"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			if(nextMode.equals("20")){
				nextAssignee = zf;
			}else if(nextMode.equals("21")){
				nextAssignee = fzsh;
			}
			break;
		case "20":
		...
		case "21":
			...
			assignee = fzsh;
			//下一步可走的流程列表
			nextModeList = new String[]{"18","19"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			if(nextMode.equals("18")){
				nextAssignee = zf;
			}else if(nextMode.equals("19")){
				nextAssignee = fzsh;
			}
			break;
		case "22":
		...
		case "29":
			...
			assignee = fycb;
			//下一步可走的流程列表
			nextModeList = new String[]{"30","31"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			if(nextMode.equals("30")){
				nextAssignee = zf;
			}else if(nextMode.equals("31")){
				nextAssignee = fycb;
			}
			break;
		case "30":
		...
		case "37":
			...
			assignee = shr;
			//下一步可走的流程列表
			nextModeList = new String[]{"9","10","11","12","13"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			if(nextMode.equals("9")||nextMode.equals("10")||nextMode.equals("11")||nextMode.equals("12")){
				//先行告知、不予处罚、撤销立案、听证告知
				nextAssignee = zf;
			}else if(nextMode.equals("13")){
				//重大案件
				nextAssignee = fzsl;
			}
			break;
		...
		case "47":
			...
			nextAssignee = zf;
			//下一步可走的流程列表
			nextModeList = new String[]{"24","23","22"};
			//随机选择一个流程
			nextMode = nextModeList[rand.nextInt(nextModeList.length)];
			break;
		case "48":
		...
		flowDTO.setFlowKey(CommonParameters.FlowKey.CASE);
		flowDTO.setBusinessId(caseInfoDTO.getCaseNum());
		flowDTO.setAssignee(assignee.getUserId());
		flowDTO.setNextAssignee(nextAssignee.getUserId());
		flowDTO.setComment(comments.get(rand.nextInt(comments.size())));
		flowDTO.setHandleMode(mode);
		this.saveTaskEntity(flowDTO, false, zf);
		if("33".equals(mode)){
			this.flowService.startFlowByKey(flowDTO);
		}else{
		...
		if(!nextMode.equals("")){
			caseRandomFlow(caseInfoDTO,zf,shr,spr,fzsl,fzsh,fzsp,fycb,fysh,fysp,tzsl,nextMode);
		}
	}
	...
	private void getCaseNameByAjxxgk() {
		...
		try {
			...
			if ( caseNameList == null || caseNameList.size() == 0 ) {
				...
				TempCase tempCase = null;
				for ( int i = 1 ; i <= 7 ; i ++ ) {
					String url = "http://www.ajxxgk.jcy.cn/html/gj/nmg/zdajxx/" + (i == 1?"index":i) + ".html";
