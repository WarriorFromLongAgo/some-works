...
//获取文件类型
function getFileType(bsrc){
	if(bsrc.indexOf('mp4') != -1 || bsrc.indexOf('mp3') != -1){
		return 'video';
	}else if(bsrc.indexOf('jpg') != -1 || bsrc.indexOf('jpeg') != -1 || bsrc.indexOf('png') != -1 || bsrc.indexOf('bmp') != -1 || bsrc.indexOf('gif') != -1){
		return 'image';
	}else if(bsrc.indexOf('pdf') != -1 || bsrc.indexOf('txt') != -1 || bsrc.indexOf('doc') != -1 || bsrc.indexOf('xls') != -1 || bsrc.indexOf('docx') != -1 || bsrc.indexOf('xlsx') != -1){
		return 'file';
	}
	return null;
}
...
function picCha(){
	...
	var ft = getFileType(bsrc);
	if(ft == 'video'){
		$('.bpic').replaceWith('<video autoplay controls="controls" class="bpic bpicvideo"></video>');
	}else if(ft == 'image'){
		$('.bpic').replaceWith('<img class="bpic bpicimg" />');
	}else if(ft == 'file'){
		$('.bpic').replaceWith('<iframe class="bpic bpicfile"></iframe>');
	}
	$('.bpic').attr('src',bsrc);
