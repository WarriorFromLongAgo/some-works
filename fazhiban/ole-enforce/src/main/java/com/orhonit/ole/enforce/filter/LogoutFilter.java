package com.orhonit.ole.enforce.filter;
...
import com.orhonit.ole.enforce.advice.EnforceExceptionHandlerAdvice.ResponseInfo;
...
public class LogoutFilter extends org.apache.shiro.web.filter.authc.LogoutFilter {
	...
	private static String SUCCESS_INFO = JsonUtil
			.toJson(ResponseInfo.builder().code(HttpStatus.OK.value() + "").msg("退出成功").build());
	private static String ERR_INFO = JsonUtil
			.toJson(ResponseInfo.builder().code(HttpStatus.BAD_REQUEST.value() + "").msg("退出失败,token不存在").build());
