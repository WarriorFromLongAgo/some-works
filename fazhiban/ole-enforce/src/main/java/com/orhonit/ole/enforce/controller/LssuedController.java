...
public class LssuedController {
	...
	public void saveCase(@RequestBody @Valid LssuedDTO lssuedDTO) {
		...
		PersonDTO pDTO = this.personDao.findUserInfo(lssuedEntity.getPersonId());
		try {
			ApiMessagePushController.send("您有一条专项任务，请点击查看...", pDTO.getCertNum(),
					CommonParameters.PushMessage.CASE_TYPE_3, lssuedEntity.getCheckNum());
		} catch (Exception e) {
	...
	public TableResponse<LssuedEntity> listCase(TableRequest request) {
		...
		String deptId = String.valueOf(request.getParams().get("userDeptId"));
		String deptIds = caseService.execFunction(deptId);
		if (StrUtil.isNotEmpty(deptIds)) {
			request.getParams().put("userDeptId", deptId + "," + deptIds);
		}
	...
	public Result<CheckRecordEntity> getCheckParty(@PathVariable String checkId) {
