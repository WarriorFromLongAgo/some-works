 ...
 */
@RestController
@RequestMapping("/specialsampling")
@Slf4j
public class SpecialSamplingController {
	
	@Autowired
	private LssuedService lssuedService;
	
	@GetMapping(value="/list")
	@ApiOperation(value = "申请抽样取证列表")
	public TableResponse<LssuedEntity> listCase(TableRequest request) {
		request.getParams().put("status", CommonParameters.CheckStatus.APPLICATION_FOR_SAMPLING);
