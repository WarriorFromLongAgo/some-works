package com.orhonit.ole.enforce.service.flow.imple;
...
import com.orhonit.ole.enforce.dao.FlowDao;
...
public class FlowServiceImpl implements FlowService {
	...
	@Override
	public List<TaskDTO> getUserTaskat(String assignee, Integer currentPage , Integer pageSize,String type) {
		return flowDao.getUserTaskat(assignee, currentPage, pageSize,type);
	}
...
//        String timeDate = DateFormatUtils.format(ca.getTime(), pattern); 
	...
	@Override
	public List<NextOpinionDTO> getHaveRoleOpinionByDeptProperty(String dept_id) {
		DeptDTO deptDTO = deptService.findDeptInfoById(dept_id);
		if(deptDTO.getDept_property() == CommonParameters.DeptProperty.WTDW){
			List<NextOpinionDTO> parent = this.flowDao.getHaveRoleOpinionByDeptId(deptDTO.getParent_id(), CommonParameters.Role.OPINION);
			List<NextOpinionDTO> own = this.flowDao.getHaveRoleOpinionByDeptId(dept_id, CommonParameters.Role.OPINION);
			parent.addAll(own);
			return parent;
		}else if(deptDTO.getDept_property() == CommonParameters.DeptProperty.SQDW || deptDTO.getDept_property() == CommonParameters.DeptProperty.XZJG){
			return this.flowDao.getHaveRoleOpinionByDeptId(dept_id, CommonParameters.Role.OPINION);
		}else
		return null;
	}
	...
	@Override
	public List<NextOpinionDTO> getHaveZfrRoleOpinionByDeptId(String deptId) {
		return this.flowDao.getHaveRoleOpinionByDeptId(deptId, CommonParameters.Role.LAW_ENFORCEMENT_OFFICIALS);
	}
	
	@Override
	public List<NextOpinionDTO> getApproveUserByDeptProperty(String dept_id) {
		DeptDTO deptDTO = deptService.findDeptInfoById(dept_id);
		if(deptDTO.getDept_property() == CommonParameters.DeptProperty.WTDW){
			List<NextOpinionDTO> parent = this.flowDao.getHaveRoleOpinionByDeptId(deptDTO.getParent_id(), CommonParameters.Role.APPROVE);
			List<NextOpinionDTO> own = this.flowDao.getHaveRoleOpinionByDeptId(dept_id, CommonParameters.Role.APPROVE);
			parent.addAll(own);
			return parent;
		}else if(deptDTO.getDept_property() == CommonParameters.DeptProperty.SQDW || deptDTO.getDept_property() == CommonParameters.DeptProperty.XZJG){
			return this.flowDao.getHaveRoleOpinionByDeptId(dept_id, CommonParameters.Role.APPROVE);
		}else
		return null;
	}
	...
	public InputStream getCoordinateAndFlowSeqByTaskId(String taskId,String key,String businessId) {
		...
		if(task == null){
			...
			if (processInstance == null) {
				...
				procinstId= historicProcessInstance.getId();
			} else {
	...
	@Override
	public List<ApiCaseTaskDTO> getCaseTask(Map<String, Object> params, Integer start, Integer length) {		
		return this.flowDao.getCaseTask(params, start, length);
	}

	@Override
	public List<ApiCheckTaskDTO> getCheckDailyTask(Map<String, Object> params, Integer start, Integer length) {
		return this.flowDao.getCheckDailyTask(params, start, length);
	}

	@Override
	public List<ApiCheckTaskDTO> getCheckTask(Map<String, Object> params,Integer start, Integer length) {
		return this.flowDao.getCheckTask(params,start, length);
	}
	...
	@Override
	public List<ApiCaseDealDTO> getPerson(String deptId) {
		return this.flowDao.getPersonByDeptId(deptId);
	}

	@Override
	public Map<String, String> getIndexCount() {
		User user = UserUtil.getCurrentUser();
		Map<String, String> resultMap=new HashMap<>();
		ApiCountDTO caseCount = this.caseService.needDealCount(String.valueOf(user.getId()));
		ApiCountDTO caseSubmitCount = this.caseService.caseSubmitCount(user.getPerson_id());
		ApiCountDTO dailyCheckCount = this.checkDailyService.needDealCount(String.valueOf(user.getId()));	
		ApiCountDTO proCheckCount = this.lssuedService.needDealCount(String.valueOf(user.getId()));
		if(caseCount!=null){
			resultMap.put("ajslsl", caseCount.getCount());
		}
		if(caseSubmitCount!=null){
			resultMap.put("ajsbsl", caseSubmitCount.getCount());
		}
		if(dailyCheckCount!=null){
			resultMap.put("rcjcsl", dailyCheckCount.getCount());
		}
		if(proCheckCount!=null){
			resultMap.put("zxjcsl", proCheckCount.getCount());
		}
		return resultMap;
	}
