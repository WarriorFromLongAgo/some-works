...
public class OwnPermissionController {
	...
	public JSONArray permissionsCurrent() {
		...
		if (list == null) {
			...
			list = this.permissionService.getOwnMenuByUserIdAndSysId(user.getId(), SystemConstants.PERMISSION_ID_ENFORCE.getCode().toString());
			UserUtil.setPermissionSession(list);
		}
		final List<Permission> permissions = list;

		return reset(permissions, SystemConstants.PERMISSION_ID_ENFORCE.getCode().toString());
	}
