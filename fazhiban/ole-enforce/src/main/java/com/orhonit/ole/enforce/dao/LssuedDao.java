...
public interface LssuedDao  {
	...
	@Update("UPDATE ole_ef_check ck SET check_title=#{checkTitle},check_object=#{checkObject},start_date=#{startDate},end_date=#{endDate},ck.STATUS=#{status},check_basis=#{checkBasis},check_content=#{checkContent},check_way=#{checkWay} WHERE ck.check_num = #{checkNum}")
	void updateBycheckNum(LssuedEntity lssuedEntity);
	
	@Delete ("DELETE FROM ole_ef_check WHERE check_num=#{checkNum}")
	void deleteByCheckNum(String checkNum);
}
