...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/opa/opaScheme/">考核方案列表</a></li>
		<shiro:hasPermission name="pa:opaScheme:edit"><li><a href="${ctx}/opa/opaScheme/form">考核方案添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="opaScheme" action="${ctx}/opa/opaScheme/" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="opaScheme">
			<tr>
				<td>
					...
					<c:if test="${opaScheme.status eq fns:getDictValue('待审核','opa_scheme_status','') or opaScheme.status eq fns:getDictValue('已审核','opa_scheme_status','')  or opaScheme.status eq fns:getDictValue('已发布','opa_scheme_status','')}">
