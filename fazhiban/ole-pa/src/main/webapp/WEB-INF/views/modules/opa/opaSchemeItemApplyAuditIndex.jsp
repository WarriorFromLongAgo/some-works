...
<head>
	...
	<script type="text/javascript">
		...
		function btnRender(row){
			var str="";
			if((row.status == "${fns:getDictValue('填报待审核', 'opa_schemeItem_status', '')}")){
					//上报
					str+="<shiro:hasPermission name='pa:opaSchemeItem:apply:audit:pass'><a href='${ctx}/opa/opaSchemeItem/apply/audit/pass?id="+row.id 
					+"&schemeId="+row.schemeId+"' onclick='return applyAuditPass(this.href)'>审核通过</a>&nbsp;</shiro:hasPermission>";
					str+="<shiro:hasPermission name='pa:opaSchemeItem:apply:audit:return'><a href='${ctx}/opa/opaSchemeItem/apply/audit/return?id="+row.id 
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/opa/opaSchemeItem/apply/audit/index">方案指标填报审核列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="opaSchemeItem" action="${ctx}/opa/opaSchemeItem/apply/audit/index" method="post" class="breadcrumb form-search">
		<ul class="ul-form">
			<li><label>方案名称：</label>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.itemId}}" pId="{{pid}}">
				<td><a href="${ctx}/opa/opaSchemeItem/apply/audit/view?id={{row.id}}">
					{{row.name}}
				</a></td>
				<td>
					{{dict.type}}
