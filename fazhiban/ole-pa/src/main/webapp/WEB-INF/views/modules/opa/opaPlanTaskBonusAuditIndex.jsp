	...
	<script type="text/javascript">
		...
		function btnRender(row){
			...
			if((row.status == "${fns:getDictValue('申请中', 'opa_planTask_status', '')}")){
					//上报
					str+="<shiro:hasPermission name='pa:opaPlanTask:bonus:audit:pass'><a href='${ctx}/opa/opaPlantask/bonus/audit/pass?id="+row.id 
						+"&planId="+row.planId+"' onclick='return bonusAuditPass(this.href)'>通过</a>&nbsp;</shiro:hasPermission>";
					str+="<shiro:hasPermission name='pa:opaPlanTask:bonus:audit:return'><a href='${ctx}/opa/opaPlantask/bonus/audit/return?id="+row.id 
					+"&planId="+row.planId+"&reason=' onclick='return bonusAuditReturn(this.href)'>退回</a>&nbsp;</shiro:hasPermission>";
					
			}
			if((row.attachedPath != null && row.attachedPath != undefined && row.attachedPath != '')){ 
				str+="<shiro:hasPermission name='pa:opaPlanTask:audit:edit'><a href='${ctx}/opa/opaPlanTask/download?id="+row.id 
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/opa/opaPlanTask/bonus/index">加减分审核</a></li>
	</ul>
