...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/opa/opaScheme/audit/index">考核方案审核列表</a></li>
		<li class="active"><a href="${ctx}/opa/opaScheme/audit/view?id=${opaScheme.id}">考核方案明细</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="opaScheme" action="${ctx}/opa/opaScheme/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			<c:if test="${opaScheme.status eq fns:getDictValue('待审核','opa_scheme_status','')}">
				<shiro:hasPermission name="pa:opaScheme:audit:pass"><input id="btnSubmit" class="btn btn-primary" type="button" value="审核通过" onclick="return confirmx('确认要通过该考核方案吗？', auditPass)"/>&nbsp;</shiro:hasPermission>
				<shiro:hasPermission name="pa:opaScheme:audit:return"><input id="btnApply" class="btn btn-primary" type="button" value="退回" onclick="return confirmx('确认要退回该考核方案吗？', auditReturn)"/>&nbsp;</shiro:hasPermission>
			</c:if>
