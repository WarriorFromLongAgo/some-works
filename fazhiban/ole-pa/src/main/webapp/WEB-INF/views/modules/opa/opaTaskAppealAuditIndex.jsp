	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			for (var i=0; i<data.length; i++){
				ids.push(data[i].planId+data[i].office.id);
			}
			ids = ',' + ids.join(',') + ',';
			for (var i=0; i<data.length; i++){
				if (ids.indexOf(','+data[i].planId+data[i].office.id+',') == -1){
					if ((','+rootIds.join(',')+',').indexOf(','+data[i].planId+data[i].office.id+',') == -1){
						rootIds.push(data[i].planId+data[i].office.id);
					}
				}
			}
		...
		function btnRender(row){
			...
			if((row.status == "${fns:getDictValue('审核中', 'opa_taskAppeal_status', '')}")){
					//上报
					str+="<shiro:hasPermission name='pa:opaTaskAppeal:audit:pass'><a href='${ctx}/opa/opaTaskAppeal/audit/pass?id="+row.id 
						+"&planId="+row.planId+"' onclick='return auditPass(this.href)'>通过</a>&nbsp;</shiro:hasPermission>";
					str+="<shiro:hasPermission name='pa:opaTaskAppeal:audit:return'><a href='${ctx}/opa/opaTaskAppeal/audit/return?id="+row.id 
			...
			if((row.attachedPath != null && row.attachedPath != undefined && row.attachedPath != '')){ 
				str+="<shiro:hasPermission name='pa:opaTaskAppeal:audit:edit'><a href='${ctx}/opa/opaPlanTask/download?id="+row.id 
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/opa/opaTaskAppeal/audit/index">申诉审核</a></li>
	</ul>
