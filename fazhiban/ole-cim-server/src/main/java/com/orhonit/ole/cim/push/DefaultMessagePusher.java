...
public class DefaultMessagePusher implements CIMMessagePusher {
	...
	public void push(Message msg) {
		System.out.println("========reveiver is ="+msg.getReceiver());
		...
		if(session!=null){
			System.out.println("session is localhost="+session.isLocalhost()+"  host="+session.getHost());
		}else{
		...
		if (session != null && session.isConnected()) {
			session.write(msg);
			System.out.println("向【"+msg.getReceiver()+"】发送了一条消息【"+msg.getContent()+"】");
			return;
		}
