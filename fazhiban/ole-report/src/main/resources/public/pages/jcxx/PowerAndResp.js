/**
 * 
 */
var powerResp = echarts.init(document.getElementById('powerResp'),'echartstheme');
var powerRespP = echarts.init(document.getElementById('powerRespP'),'echartstheme');
$(function() {
	
	
	/**
	 * 所有行政部门职权数量
	 */
	var paperDataURI = 'data:image/png;base64,';

	poption = {
		    //backgroundColor: '#0f375f',
		    tooltip: {},
		    title:{text:'权责分析',textStyle:{color:'#ddd'}},
		    legend: {
		        data: ['所有行政部门职权数量'],
		        textStyle: {color: '#ddd',fontSize:16}
		    },
		    xAxis: [{
		        data: ['所有行政部门职权数量', '', '珠穆朗玛\nQomolangma'],//, '乞力马扎罗\nKilimanjaro'],
		        axisTick: {show: false},
		        axisLine: {show: false},
		        axisLabel: {margin: 30,color: '#add',fontSize: 14}
		    }],
		    yAxis: {
		    	splitLine: {show: false},axisTick: {show: false},
		    	axisLine: {show: false},axisLabel: {show: false}
		    },
		    markLine: {z: -1},
		    animationEasing: 'elasticOut',//初始动画的缓动效果。
		    series: [{
		        type: 'pictorialBar',
		        name: '所有行政部门职权数量',
		        hoverAnimation: true,
		        label: {
		            normal: {
		            	show: true,position: 'top',formatter: '{c} m',textStyle: {fontSize: 15,color: '#add'}
		            }},
		        data: [{
		        	value: (function(){
		            	var data;
		            	$.ajax({
		            		url:'/report/power/getTotal',type:'GET',async : false, //同步执行
		            		success:function(result){
		            			data = result[0].count;
		            		},
		            		error:function(errMsg){
		            			alert("请求不到数据");
		            			powerResp.hideLoading('请求数据中....');
		            		}
		            	});
		            	return data;
		            })(),
		            symbol: 'image://' + paperDataURI,
		            symbolRepeat: true,
		            symbolSize: ['130%', '20%'],
		            symbolOffset: [0, 10],
		            symbolMargin: '-30%',
		            animationDelay: function (dataIndex, params) {return params.index * 30;}
		        }, 
		        {value: '-',symbol: 'none',}, 
		        {
		            value: 8844,
		            symbol: 'image://../../img/power/hill-Qomolangma.png',
		            symbolSize: ['200%', '105%'],
		            symbolPosition: 'end',
		            z: 10
		        },
//		        {
//		            value: 5895,
//		            symbol: 'image://../../img/power/hill-Kilimanjaro.png',
//		            symbolSize: ['200%', '105%'],
//		            symbolPosition: 'end'
//		        }
		        ],
		        markLine: {
		            symbol: ['none', 'none'],
		            label: {normal: {show: false}},
		            lineStyle: {normal: {color: '#e54035',width: 2}},
		            data: [{yAxis: 8844}]
		        }
		    }, {
		        name: 'all',
		        type: 'pictorialBar',
		        barGap: '-100%',
		        symbol: 'circle',
		        itemStyle: {normal: {color: '#185491'}},
		        silent: true,
		        symbolOffset: [0, '50%'],
		        z: -10,
		        data: [
		        	{value: 1,symbolSize: ['150%', 50]}, 
		        	{value: '-'}, 
		        	{value: 1,symbolSize: ['200%', 50]},
		        	//{value: 1,symbolSize: ['200%', 50]}
		        	]
		    }]
		};
	powerResp.setOption(poption);
	
	/**
	 * 各个行政执法类型的行政执法职权数
	 */
	
	qoption = {
		    tooltip: {
		        trigger: 'item',
		        formatter: "{a} <br/>{b}: {c} ({d}%)"
		    },
		    legend: {
		        orient: 'vertical',
		        x: 'left',
		        top:'5%',
		        left :'5%',
		        data:[]
		    },
		    series: [
		        {
		            name:'行政执法职权数',
		            type:'pie',
		            radius: ['40%', '55%'],
		            label: {
		                /*normal: {
		                	
		                    formatter: '{a|{a}}\n{hr|}\n  {b|{b}：}{c}  {per|{d}%}  ',
		                    backgroundColor: '#eee',
		                    borderColor: '#aaa',
		                    borderWidth: 1,
		                    borderRadius: 4,
		                    // shadowBlur:3,
		                    // shadowOffsetX: 2,
		                    // shadowOffsetY: 2,
		                    // shadowColor: '#999',
		                    // padding: [0, 7],
		                    rich: {
		                        a: {
		                            color: '#999',
		                            lineHeight: 22,
		                            align: 'center'
		                        },
		                        hr: {
		                            borderColor: '#aaa',
		                            width: '100%',
		                            borderWidth: 0.5,
		                            height: 0
		                        },
		                        b: {
		                            fontSize: 16,
		                            lineHeight: 33
		                        },
		                        per: {
		                            color: '#eee',
		                            backgroundColor: '#334455',
		                            padding: [2, 4],
		                            borderRadius: 2
		                        }
		                    }
		                },*/
		                /*emphasis: {
		                    show: true,
		                    textStyle: {
		                        fontSize: '30',
		                        fontWeight: 'bold'
		                    }
		                }*/
		            },
		            data:[
		                
		            ]
		        }
		    ]
		};

	powerRespP.setOption(qoption);
	
	$.get('/report/power/getClassiFication').done(function(data){
		powerRespP.setOption({
			legend: {
		        data:data.legned
		    },
			series:[{
				type:'pie',
				data:data.data
			}]
		});
	});
	
});