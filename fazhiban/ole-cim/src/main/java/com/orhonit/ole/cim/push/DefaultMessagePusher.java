...
public class DefaultMessagePusher implements CIMMessagePusher {
	...
	public void push(Message msg) {
		 ...
		 */
		if(session==null){
			 System.out.println("session is null,host=192.168.3.100");
			 System.out.println("session is null,receiver is ="+msg.getReceiver());
			 MessageDispatcher.execute(msg,"192.168.3.100");
			 return; 
		}else{
			System.out.println("session is localhost="+session.isLocalhost());
		    if(session!=null&&!session.isLocalhost()){
				 //判断当前session是否连接于本台服务器，如不是
				 //发往目标服务器处理 
		    	 System.out.println("session is localhost");
				 MessageDispatcher.execute(msg,session.getHost());
				return; 
			}
		    System.out.println("session is not  localhost");
