...
public class AppTokenUtil {
	...
	public static String createToken(String persosnId, String expireSecond) {
		long currentTime = System.currentTimeMillis();
		long expireTime = currentTime + (Integer.valueOf(expireSecond) * 1000);
		String random = AppTokenUtil.genRandomNum(RANDOM_LENGTH);
		String content = persosnId + "|" + currentTime + "|" + expireTime + "|" + random;
		String token =  AppTokenUtil.encrypt(content,KEY);
		try {
	...
	public static String getPersonIdByToken(String token) {
		
		if ( StringUtils.isEmpty(token)) {
			throw new RuntimeException("token为空.");
		}
		
		String result = AppTokenUtil.decrypt(token , KEY);
		...
		log.info("the personId is {}", personId);
	...
	public static void main(String[] args) throws Exception{
		...
		String token = AppTokenUtil.createToken("1", "30");
